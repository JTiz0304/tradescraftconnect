...
const userTypes = [
  { id: 'gc_builder', label: 'GC / Builder', icon: '🏗️', desc: 'I hire companies or tradespeople for projects' },
  { id: 'business_owner', label: 'Trade Business Owner', icon: '🔧', desc: 'I run a trade company and look for work opportunities' },
  { id: 'professional', label: 'Trades Professional', icon: '👷', desc: 'I am an individual tradesperson looking for employment' },
  { id: 'apprentice', label: 'Apprentice', icon: '🎓', desc: 'I am learning a trade and looking for experience or OJT' },
]
...
export default function OnboardingPage() {
  ...
  const handleSave = async () => {
    ...
    if (!formData.full_name?.trim() || !formData.location?.trim()) {
      setMessage('Please enter your full name and location.')
      return
    }
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white flex items-center justify-center p-4 sm:p-6">
      <div className="w-full max-w-lg">

        {step === 1 && (
          <>
            <h1 className="text-2xl font-bold text-center mb-2">Welcome! What best describes you?</h1>
            <p className="text-gray-400 text-center mb-8">Choose how you will primarily use TradesCraftConnect. You can build out your full profile next.</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {userTypes.map(type => (
                <button
                  key={type.id}
                  onClick={() => handleTypeSelect(type.id)}
                  className="bg-gray-900 border border-gray-700 hover:border-orange-500 rounded-2xl p-5 sm:p-6 text-left transition"
                >
        ...
        {step === 2 && (
          <div className="mb-5 flex items-center justify-between gap-4 rounded-xl border border-gray-800 bg-gray-900 px-4 py-3">
            <div className="text-sm text-gray-300">Setting up as <span className="font-semibold text-orange-400">{selectedType?.label}</span></div>
            <button onClick={() => { setStep(1); setMessage('') }} className="shrink-0 text-sm text-gray-400 underline hover:text-white">Change role</button>
          </div>
        )}
        ...
        {step === 2 && userType === 'gc_builder' && (
          <>
            ...
            <button onClick={handleSave} disabled={saving} className="w-full bg-orange-500 hover:bg-orange-600 disabled:cursor-not-allowed disabled:opacity-60 text-white font-semibold py-3 rounded-xl transition mt-2">{saving ? 'Saving...' : 'Complete Setup'}</button>
          </>
        )}
